...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: joincoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: joincoind.pid)") + "\n";
    ...
    strUsage += "  -tor=<ip:port>         " + _("Use proxy to reach tor hidden services") + "\n";
    strUsage += "  -dns                   " + _("Allow DNS lookups for -addnode, -seednode and -connect") + "\n";
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 17941 or testnet: 27941") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 17942 or testnet: 27942)") + "\n";
    ...
    strUsage += "  -algo=<algo>           " + _("Mining algorithm: scrypt, x13") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Algo
    std::string strAlgo = GetArg("-algo", "bastion");
    ...
    if (strAlgo == "sha")
        miningAlgo = ALGO_SHA;
         ...
         else if (strAlgo == "groestle")
                miningAlgo = ALGO_GROESTLE;
	 else if (strAlgo == "pentablake")
	        miningAlgo = ALGO_PENTABLAKE;
	 else if (strAlgo == "whirlpool")
	        miningAlgo = ALGO_WHIRLPOOL;
	 else if (strAlgo == "luffa")
	        miningAlgo = ALGO_LUFFA;
	 else if (strAlgo == "keccak")
	        miningAlgo = ALGO_KECCAK;
	 else if (strAlgo == "quark")
        	 miningAlgo = ALGO_QUARK;
         else if (strAlgo == "bastion")
                 miningAlgo = ALGO_BASTION;
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Joincoin is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Joincoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Joincoin server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Joincoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Joincoin to complete") << "\n";
