...
enum
{
    ...
    BLOCK_VERSION_X13           = 4,
    BLOCK_VERSION_X15           = 5,
    BLOCK_VERSION_SCRYPT           = 6,
    BLOCK_VERSION_NIST5           = 7,
    BLOCK_VERSION_GROESTLE           = 8,
    BLOCK_VERSION_PENTABLAKE           = 9,
    BLOCK_VERSION_WHIRLPOOL           = 10,
    BLOCK_VERSION_LUFFA           = 11,
    BLOCK_VERSION_KECCAK           = 12,
    BLOCK_VERSION_QUARK           = 13,
    BLOCK_VERSION_BASTION         = 15
};
...
inline int GetAlgo(int nVersion)
{
	...
	case BLOCK_VERSION_NIST5:
	  return ALGO_NIST5;
	case BLOCK_VERSION_GROESTLE:
	  return ALGO_GROESTLE;
	case BLOCK_VERSION_PENTABLAKE:
	  return ALGO_PENTABLAKE;
	case BLOCK_VERSION_WHIRLPOOL:
	  return ALGO_WHIRLPOOL;
	case BLOCK_VERSION_LUFFA:
	  return ALGO_LUFFA;
	case BLOCK_VERSION_KECCAK:
	  return ALGO_KECCAK;
	case BLOCK_VERSION_QUARK:
	  return ALGO_QUARK;
        case BLOCK_VERSION_BASTION:
          return ALGO_BASTION;
...
inline std::string GetAlgoName(int Algo)
{
	...
	case ALGO_GROESTLE:
	  return std::string("groestle");
	case ALGO_PENTABLAKE:
	   return std::string("pentablake");
	case ALGO_WHIRLPOOL:
	   return std::string("whirlpool");
	case ALGO_LUFFA:
	   return std::string("luffa");
	case ALGO_KECCAK:
	   return std::string("keccak");
	case ALGO_QUARK:
	   return std::string("quark");
        case ALGO_BASTION:
           return std::string("bastion");
...
public:
    ...
    uint256 GetPoWHash(int algo) const
    {
            ...
            case ALGO_SCRYPT:
		{
            ...
            case ALGO_GROESTLE:
                return HashGROESTLE(BEGIN(nVersion), END(nNonce));
            case ALGO_PENTABLAKE:
                return HashPENTABLAKE(BEGIN(nVersion), END(nNonce));
            case ALGO_WHIRLPOOL:
                return HashWHIRLPOOL(BEGIN(nVersion), END(nNonce));
            case ALGO_LUFFA:
                return HashLUFFA(BEGIN(nVersion), END(nNonce));
            case ALGO_KECCAK:
                return HashKECCAK(BEGIN(nVersion), END(nNonce));           
            case ALGO_QUARK:
                return HashQUARK(BEGIN(nVersion), END(nNonce));
            case ALGO_BASTION:
                return HashBASTION(BEGIN(nVersion), END(nNonce));
